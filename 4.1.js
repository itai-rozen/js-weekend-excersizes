...
const getFibonacciN = (startingPoint,n) => {

    let fibonacciArr = [startingPoint,1]
    if (n < 3) return fibonacciArr[n-1]

    for (let i = 2 ; i < n; i++){
        fibonacciArr.push(fibonacciArr[i-1] + fibonacciArr[i-2])
    }
    console.log(`fibonacci series until the ${n} element: `,fibonacciArr)
    return fibonacciArr[fibonacciArr.length-1]
}

console.log('expected: 3. got: ', getFibonacciN(0,5))
console.log('expected: 5. got: ', getFibonacciN(1,5))
