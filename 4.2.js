...
const getTribonacciN = (signature,n) => {
    let tribonacciArr = [...signature]
    console.log(`input: signture - ${signature}. N - ${n}`)

    if (n === 0) return []
    if (n < 3) return tribonacciArr[n-1]
    for (let i = 3 ; i < n; i++){
        tribonacciArr.push(tribonacciArr[i-1] +tribonacciArr[i-2] + tribonacciArr[i-3] )
    }
    console.log(tribonacciArr)
    return tribonacciArr[tribonacciArr.length-1]
}
console.log('expected: 24. got: ',getTribonacciN([0,0,1],9))
console.log('expected: 81. got: ',getTribonacciN([1,1,2],9))
